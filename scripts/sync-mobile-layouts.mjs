...
import { parseLayoutJson } from "../src/layout_semantics.js";
...
for (const [key, metadata] of Object.entries(BUILTIN_LAYOUTS)) {
  const definition = parseLayoutJson(await readFile(path.join(projectRoot, "src", metadata.file), "utf8"));
...
const presentation = (await readFile(canonicalPresentationPath, "utf8"))
  .replace('from "./layout_semantics.js"', 'from "./layout_semantics.generated.js"');
await writeFile(mobilePresentationPath, `// Generated by \`npm run sync:mobile-layouts\`; do not edit by hand.\n${presentation}`);
